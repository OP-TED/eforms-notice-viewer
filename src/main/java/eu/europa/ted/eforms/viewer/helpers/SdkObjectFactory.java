...
public class SdkObjectFactory {
  ...
  public <T> T getComponentImpl(String sdkVersion, SdkComponentTypeEnum componentType, Class<T> intf) throws InstantiationException {
    Map<SdkComponentTypeEnum, SdkComponentDescriptor<?>> components = componentsMap.get(sdkVersion);
    if (components == null) {
      log.warn("No SDK components found for SDK [{}]. Trying with fallback SDK ({})", sdkVersion, SdkComponent.ANY);
      sdkVersion = SdkComponent.ANY;
      components = componentsMap.get(sdkVersion);
      if (components == null) {
        throw new IllegalArgumentException(MessageFormat.format("No SDK component implementations found for SDK version [{0}].", sdkVersion));
      }
    }
    SdkComponentDescriptor<T> descriptor = (SdkComponentDescriptor<T>) components.get(componentType);
    if (descriptor == null) {
      throw new IllegalArgumentException(MessageFormat.format("No implementation found for component type [{0}] of SDK [{1}]", componentType, sdkVersion));
    }
    return descriptor.createInstance();
  }

  @SuppressWarnings("unchecked")
  public static SdkMap<SdkCodelist> getCodelistsMap(String sdkVersion, Path codelistsPath) throws InstantiationException, IOException {
    return SdkObjectFactory.INSTANCE.getComponentImpl(sdkVersion, SdkComponentTypeEnum.CODELIST_MAP, SdkMap.class).setResourceFilepath(codelistsPath);
  }

  @SuppressWarnings("unchecked")
  public static SdkMap<SdkField> getFieldsMap(String sdkVersion, Path jsonPath) throws InstantiationException, IOException {
    return SdkObjectFactory.INSTANCE.getComponentImpl(sdkVersion, SdkComponentTypeEnum.FIELD_MAP, SdkMap.class).setResourceFilepath(jsonPath);
  }

  @SuppressWarnings("unchecked")
  public static SdkMap<SdkNode> getNodesMap(String sdkVersion, Path jsonPath) throws InstantiationException, IOException {
    return SdkObjectFactory.INSTANCE.getComponentImpl(sdkVersion, SdkComponentTypeEnum.NODE_MAP, SdkMap.class).setResourceFilepath(jsonPath);
  }
}
