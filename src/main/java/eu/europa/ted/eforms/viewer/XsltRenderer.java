...
public class XsltRenderer extends IndentedStringWriter implements Renderer {
    ...
    public String renderValueReference(String valueReference) {
        return String.format("<span class=\"value\"><xsl:value-of select=\"%s\"/></span>", valueReference);
    }

    @Override
    public String renderLabelFromKey(final String key) {
        return String.format("<span class=\"label\"><xsl:value-of select=\"($labels/properties/entry[./@key='%s']/text(), ' Label not found (%s)')[1]\"/></span>", key, key);
    }
    ...
    public String renderLabelFromExpression(final String expression) {
        ...
        writer.writeLine(String.format("<xsl:variable name=\"%s\" select=\"%s\"/>", variableName, expression));
        writer.writeLine(String.format("<span class=\"dynamic-label\"><xsl:value-of select=\"($labels/properties/entry[./@key=$%s]/text(), concat('Label not found (', $%s, ')'))[1]\"/></span>", variableName, variableName));
        return writer.toString();
    }
