...
public class NoticeViewer {
  ...
  /**
   * @param language
   *          The language as a two letter code
   * @param noticeXmlFilename
   *          The notice xml filename but without the xml extension
   * @param viewIdOpt
   *          An optional SDK view id to use, this can be used to enforce a
   *          custom view like notice summary. It could fail if this custom view
   *          is not compatible with the notice sub type
   * @param sdkResourcesVersion
   *          The version to use when loading SDK resources
   * @param sdkResourcesRoot
   *          (Optional) The root folder of the SDK resources. If not specified,
   ...
   * @throws SAXException
   *           XML parse related errors
   */
  public static Path generateHtml(final String language, final Path noticeXmlPath, final Optional<String> viewIdOpt) throws IOException, SAXException, ParserConfigurationException {
    ...
    if (noticeSubTypeFromXmlOpt.isEmpty()) {
      throw new RuntimeException(String.format("SubTypeCode not found in notice xml: %s", noticeXmlPath));
    }
    ...
    if (eformsSdkVersionOpt.isEmpty()) {
      throw new RuntimeException(String.format("eForms SDK version not found in notice xml: %s", noticeXmlPath));
    }
    // Build XSL from EFX.
    ...
    logger.info("noticeSubType={}, viewId={}, eformsSdkVersion={}", noticeSubType, viewId, eformsSdkVersion);
   ...
   * @param language
   *          The language as a two letter code
   * @param noticeXmlContent
   *          The notice xml content
   * @param xsl
   *          structure of the notice
   * @param charset
   *          of the input string content (xml notice and xsl structure) and
   *          output html string
   * @param viewIdOpt
   *          An optional SDK view id to use, this can be used to enforce a
   *          custom view like notice summary. It could fail if this custom view
   ...
   *           XML parse related errors
   */
  public static String generateHtml(final String language, final String noticeXmlContent, final String xsl, final Charset charset, final Optional<String> viewIdOpt) throws IOException, SAXException, ParserConfigurationException {
    ...
    try (
        final ByteArrayInputStream noticeXmlInputStream = new ByteArrayInputStream(noticeXmlContent.trim().getBytes(charset));
   ...
   *          The language as a two letter code
   * @param noticeXmlContent
   *          The notice xml content as InputStream
   * @param xslIs
   *          structure of the notice as InputStream
   * @param charset
   ...
   *           XML parse related errors
   */
  public static String generateHtml(final String language, final ByteArrayInputStream noticeXmlContent, final ByteArrayInputStream xslIs, final Charset charset, final Optional<String> viewIdOpt) throws IOException, ParserConfigurationException, SAXException {
    try (final ByteArrayOutputStream baos = new ByteArrayOutputStream();
      ...
      try (
        ...
        applyXslTransform(language, eformsSdkVersion, new StreamSource(noticeXmlIsClone2), new StreamSource(xslIs), htmlResult);
        // Ensure the HTML can be parsed.
  ...
  public static String generateHtmlForUnitTest(final String language, final String noticeXmlContent, final String xsl, final Charset charset, final Optional<String> viewIdOpt) {
    try {
  ...
  static Path applyXslTransform(final String language, String sdkVersion, final Path noticeXmlPath, final Path xslPath, final String viewId) throws IOException {
    // XML as input.
  ...
  static void applyXslTransform(final String language, String sdkVersion, final Source xmlInput, final Source xslSource, final StreamResult outputTarget) {
    try {
      // https://www.saxonica.com/documentation11/#!using-xsl/embedding
      // Use Saxon HE so that we can evaluate XSL 2.0:
      System.setProperty("javax.xml.transform.TransformerFactory", "net.sf.saxon.TransformerFactoryImpl"); // Use
                                                                                                           // the
                                                                                                           // "net.sf.saxon"
                                                                                                           // we
                                                                                                           // have
   ...
   *
   * @param viewId
   *          Something like "1" or "X02", it will try to get the corresponding
   *          view template from SDK by using naming conventions
   * @param sdkVersion
   *          The version of the desired SDK
   * @param sdkResourcesVersion
   *          The version to use when loading SDK resources
   * @param sdkResourcesRoot
   *          (Optional) The root folder of the SDK resources. If not specified,
   ...
   */
  public static final Path buildXsl(final String viewId, final String sdkVersion) throws IOException {
    ...
    try (InputStream viewInputStream = Files.newInputStream(viewPath)) {
      final String translation = EfxTranslator.translateTemplate(viewInputStream, new DependencyFactory(), sdkVersion);
  ...
  /**
   * @param viewId
   *          It can correspond to a view id, as long as there is one view id
   *          per notice id, or something else for custom views
   * @param sdkVersion
   *          The SDK version to load the path from
   */
